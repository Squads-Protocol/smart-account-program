...
use crate::{state::policies::implementations::InternalFundTransferPolicyCreationPayload, ProgramInteractionPolicyCreationPayload, SettingsChangePayload, SettingsChangePolicyCreationPayload, SpendingLimitPayload, SpendingLimitPolicyCreationPayload};
...
impl PolicyCreationPayload {
    ...
    pub fn size(&self) -> usize {
        ...
        1 + match self {
            PolicyCreationPayload::InternalFundTransfer(payload) => {
                payload.creation_payload_size()
            }
            PolicyCreationPayload::SpendingLimit(payload) => {
                payload.creation_payload_size()
            }
            PolicyCreationPayload::SettingsChange(payload) => {
                payload.creation_payload_size()
            }
            PolicyCreationPayload::ProgramInteraction(payload) => {
                payload.creation_payload_size()
            }
        }
    }
