...
describe("Instructions / batch_transaction_account_close", () => {
  ...
  it("error: wrong rent collector", async () => {
    ...
    // Create a batch.
    const batchIndex = 1n;
    let signature = await smartAccount.rpc.createBatchV2({
      connection,
    ...
    signature = await smartAccount.rpc.addTransactionToBatchV2({
      connection,
      feePayer: members.proposer,
      settingsPda,
      batchIndex: batchIndex,
    ...
    signature = await smartAccount.rpc.activateProposalV2({
      connection,
      feePayer: members.proposer,
      settingsPda,
      transactionIndex: batchIndex,
