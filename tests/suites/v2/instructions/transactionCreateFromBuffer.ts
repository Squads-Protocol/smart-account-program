...
describe("Instructions / transaction_create_from_buffer", () => {
  ...
  it("error: out of memory (no allocator)", async () => {
    ...
    const fourthSignature = await smartAccount.rpc.createProposalV2({
      connection,
    ...
    const fifthSignature = await smartAccount.rpc.approveProposalV2({
      connection,
      feePayer: members.almighty,
      settingsPda,
      transactionIndex: 1n,
    ...
    const executeIx = await smartAccount.instructions.executeTransactionV2({
      connection,
      settingsPda,
      transactionIndex: 1n,
