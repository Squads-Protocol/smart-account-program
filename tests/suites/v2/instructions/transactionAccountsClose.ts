...
describe("Instructions / transaction_accounts_close", () => {
  ...
  before(async () => {
    ...
    signature = await smartAccount.rpc.createTransactionV2({
      connection,
    ...
    signature = await smartAccount.rpc.createSettingsTransactionV2({
      connection,
      feePayer: members.proposer,
      settingsPda,
    ...
    // Execute the transaction.
    signature = await smartAccount.rpc.executeTransactionV2({
      connection,
    ...
    signature = await smartAccount.rpc.rejectProposalV2({
      connection,
      feePayer: members.voter,
      settingsPda,
      transactionIndex: rejectedTransactionIndex,
    ...
    signature = await smartAccount.rpc.cancelProposalV2({
      connection,
      feePayer: members.voter,
      settingsPda,
      transactionIndex: cancelledTransactionIndex,
  ...
  it("error: wrong rent collector", async () => {
    ...
    let signature = await smartAccount.rpc.createTransactionV2({
      connection,
      feePayer: members.proposer,
      settingsPda,
      transactionIndex,
  ...
  it("close accounts for Cancelled transaction", async () => {
    ...
    const preBalanceVoter = await connection.getBalance(
    ...
    const postBalanceVoter = await connection.getBalance(
      members.voter.publicKey
    );
    const postBalance = await connection.getBalance(members.proposer.publicKey);
    assert.equal(postBalance > preBalance, true);
