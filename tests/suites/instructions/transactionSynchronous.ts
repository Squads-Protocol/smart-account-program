...
describe("Instructions / transaction_execute_sync", () => {
  ...
  it("execute synchronous transfer from vault with lookup table", async () => {
    ...
    const [createLookupTableInstruction, lookupTablePublickey] =
      AddressLookupTableProgram.createLookupTable({
        ...
        recentSlot:
          (await connection.getLatestBlockhashAndContext()).context.slot - 5,
      });
    ...
    const createTableSignature = await connection.sendRawTransaction(
      createTableTransaction.serialize(),
      { skipPreflight: true }
    );
    ...
    const lookupTableAccount = (
      await connection.getAddressLookupTable(lookupTablePublickey)
    ).value;
  ...
  it("can create a token mint", async () => {
    ...
    const { instructions: instruction_bytes, accounts: instruction_accounts } =
      smartAccount.utils.instructionsToSynchronousTransactionDetails({
        vaultPda,
        members: [
          members.proposer.publicKey,
