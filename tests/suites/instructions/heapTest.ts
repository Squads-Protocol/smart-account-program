...
import { HeapTestInstructionArgs } from "@sqds/smart account/lib/generated";
...
describe("Instructions / transaction_buffer_close", () => {
  it("heap test", async () => {
    ...
    const heapTestIx = smartAccount.generated.createHeapTestInstruction(
      {
    ...
    const heapTestSig = await connection.sendRawTransaction(
      heapTestTx.serialize(),
      { skipPreflight: true }
    );
