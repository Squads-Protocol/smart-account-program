...
// Copy-pasted from borsh::de::hint.
#[cfg(feature = "borsh")]
mod hint {
    #[inline]
...
/// Helper method to serialize a slice of data (without the length marker).
...
mod test {
    ...
    mod deserialize {
        ...
        fn test_length_u8_type_pubkey() {
            let pubkey1 = Address::new_unique();
            let pubkey2 = Address::new_unique();
            let mut input = &[
                &[0x02][..],
                &pubkey1.to_bytes()[..],
                &pubkey2.to_bytes()[..],
            ...
            .concat()[..];
            let small_vec: SmallVec<u8, Address> = SmallVec::deserialize(&mut input).unwrap();
            assert_eq!(small_vec.0, vec![pubkey1, pubkey2]);
        }

        #[test]
        fn test_length_u16_type_u8() {
            let mut input = &[0x02, 0x00, 0x05, 0x09][..];
        ...
        fn test_length_u16_type_pubkey() {
            ...
            .concat()[..];
            let small_vec: SmallVec<u16, Address> = SmallVec::deserialize(&mut input).unwrap();
            assert_eq!(small_vec.0, vec![pubkey1, pubkey2]);
        }
    }
    ...
    mod serialize {
        ...
        fn test_length_u8_type_u8() {
            ...
            assert_eq!(output, vec![0x02, 0x03, 0x05]);
        }
        ...
        fn test_length_u8_type_u32() {
            ...
            assert_eq!(
                output,
                vec![0x02, 0x03, 0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00]
            );
        }
        ...
        fn test_length_u8_type_pubkey() {
            let pubkey1 = Address::new_unique();
            let pubkey2 = Address::new_unique();
            let small_vec = SmallVec::<u8, Address>::from(vec![pubkey1, pubkey2]);
            let mut output = vec![];
            small_vec.serialize(&mut output).unwrap();
            assert_eq!(
                output,
        ...
        fn test_length_u16_type_u8() {
            ...
            let mut output = vec![];
            small_vec.serialize(&mut output).unwrap();
            assert_eq!(output, vec![0x02, 0x00, 0x03, 0x05]);
        }
        ...
        fn test_length_u16_type_pubkey() {
            let pubkey1 = Address::new_unique();
            let pubkey2 = Address::new_unique();
            let small_vec = SmallVec::<u16, Address>::from(vec![pubkey1, pubkey2]);
            let mut output = vec![];
            small_vec.serialize(&mut output).unwrap();
            assert_eq!(
                output,
