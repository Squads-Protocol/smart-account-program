...
//! `solana-program` / `solana-instruction` dependency — only `solana-address`,
...
use crate::{PROGRAM_ID, instructions::*};
...
pub struct CreateTransactionBufferAccounts {
    pub consensus_account: Address,
    pub transaction_buffer: Address,
    pub creator: Address,
    pub rent_payer: Address,
}
...
pub struct UseSpendingLimitAccounts {
    ...
    pub system_program: Option<Address>,
    pub mint: Option<Address>,
    pub smart_account_token_account: Option<Address>,
...
pub fn use_spending_limit_ix(
    ...
    let opt_readonly = |pk: Option<Address>| match pk {
        Some(k) => AccountMeta::new_readonly(k, false),
        None => AccountMeta::new_readonly(PROGRAM_ID, false),
    };
    let opt_writable = |pk: Option<Address>| match pk {
...
pub struct CloseSettingsTransactionAccounts {
    ...
    pub proposal: Address,
    pub transaction: Address,
    pub proposal_rent_collector: Address,
    pub transaction_rent_collector: Address,
}
...
pub struct CloseBatchAccounts {
    ...
    pub batch: Address,
    pub proposal_rent_collector: Address,
    pub batch_rent_collector: Address,
}
