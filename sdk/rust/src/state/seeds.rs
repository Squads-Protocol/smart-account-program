...
#[cfg(not(feature = "testing"))]
pub const HOOK_AUTHORITY_PUBKEY: Address = Address::new_from_array([
...
pub fn get_policy_signer_seeds(settings_key: &Address, policy_seed: u64) -> Vec<Vec<u8>> {
    vec![
        SEED_PREFIX.to_vec(),
        SEED_POLICY.to_vec(),
...
mod tests {
    ...
    fn test_hook_authority_pubkey() {
        let address =
            Address::create_program_address(&[SEED_HOOK_AUTHORITY], &crate::PROGRAM_ID).unwrap();
    ...
    fn test_testing_hook_authority_pubkey() {
        let test_program_id =
            Address::try_from("GyhGAqjokLwF9UXdQ2dR5Zwiup242j4mX4J1tSMKyAmD").unwrap();
        let address =
            Address::create_program_address(&[SEED_HOOK_AUTHORITY], &test_program_id).unwrap();
        assert_eq!(address, HOOK_AUTHORITY_PUBKEY);
    }
}
