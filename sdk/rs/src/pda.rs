use squads_multisig_program::{
    SEED_EPHEMERAL_SIGNER, SEED_PREFIX, SEED_PROGRAM_CONFIG, SEED_PROPOSAL, SEED_SETTINGS,
    SEED_SMART_ACCOUNT, SEED_SPENDING_LIMIT, SEED_TRANSACTION,
};
...
pub fn get_multisig_pda(create_key: &Pubkey, program_id: Option<&Pubkey>) -> (Pubkey, u8) {
    Pubkey::find_program_address(
        &[SEED_PREFIX, SEED_SETTINGS, create_key.to_bytes().as_ref()],
        program_id.unwrap_or(&squads_multisig_program::ID),
    )
}
