...
The smart account program is a fork of the now immutable Squads Protocol v4
program. It builds on all the Squads Protocol v4 functionality and adds
synchronous methods to aid in the UX of multi-signer and gas abstracted transactions.
...
- The `@sqds/smart_account` Typescript SDK to interact with the smart account program.
- The `squads-smart-account` crate to interact with the smart account program in Solana programs as well as Rust client applications.

## Program (Smart contract) Addresses

The Squads Protocol v4 program is deployed to:

- Solana Mainnet-beta: `N/A`
- Solana Devnet: `SMRTe6bnZAgJmXt9aJin7XgAzDn1XMHGNy95QATyzpk`
...
- Rust crate: [squads-smart-account-program](https://crates.io/crates/squads-multisig-program)
- Typescript SDK: [@sqds/smart-account](https://www.npmjs.com/package/@sqds/multisig)
...
Smart Account Program is currently not audited yet.

## License

The primary license for Squads Smart Account Program 0.1 is the Business Source License 1.1 (`BUSL-1.1`), see [LICENSE](./LICENSE). The following exceptions are not licensed under the BULS-1.1, but are licensed separately as follows:
